...
let user = {
  ...
  setUserLastAction(lastAction){
    tUsers.use().getRange(this.rowInTable, tUsers.getCol(tUsers.last_action_Title)+1).setValue(lastAction);
  },
  setUserLastActionTime(lastActionTime){
    tUsers.use().getRange(this.rowInTable, tUsers.getCol(tUsers.last_action_time_Title)+1).setValue(lastActionTime);
  },
  addUserInteraction(){
    tUsers.use().getRange(this.rowInTable, tUsers.getCol(tUsers.total_interactions_Title)+1).setValue(
      tUsers.use().getRange(this.rowInTable, tUsers.getCol(tUsers.total_interactions_Title)+1).getValue() + 1
    );
  },
};
